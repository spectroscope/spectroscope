...
const EVENTS: RunEvent[] = [
  ...
  },
  {
    ...
    count: 2,
    evidence: "two steps open",
    ts: 1,
  },
  {
    type: "no_progress",
    agentId: "main",
    detector: "a_net_from_the_future",
    count: 9,
  ...
  },
  {
    ...
    output: "",
    durationMs: 1,
    evidence: "no command ran",
    ts: 1,
  },
  {
    type: "goal_check",
    agentId: "main",
    outcome: "a_new_verdict",
    command: "npm test",
    exitCode: null,
    judge: "exit_code",
    output: "",
...
describe("the run-ended line composes, in both languages", () => {
  it.each(["max_turns", "aborted", "unfinished_after_continuations", "a_reason_from_the_future"])(
    ...
    (reason) => {
      const line = reduce(initialState, { type: "run_end", runId: "r", stopReason: reason, ts: 1 }).turns.at(
        -1,
      );
...
describe("the chat actually substitutes the second sentence", () => {
  it("translates infoRefKey rather than passing it through as a var", () => {
    const chat = readFileSync(fileURLToPath(new URL("../components/Chat.tsx", import.meta.url)), "utf8");
