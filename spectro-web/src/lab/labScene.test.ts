...
describe("labScene", () => {
  it("a plan event leaves the map inert (unknown to the scene)", () => {
    ...
    const after = advanceScene(before, {
      type: "plan",
      agentId: "main",
  ...
  it("turn_start and deltas focus the llm", () => {
    expect(play([runStart("anthropic"), { type: "turn_start", agentId: "main", turn: 1, ts: T }]).focus).toBe(
      "llm",
    );
    expect(
      play([runStart("anthropic"), { type: "text_delta", agentId: "main", text: "…", ts: T }]).focus,
    ).toBe("llm");
  });
  ...
  it("write_file focuses the disk, writes, and shows the file's basename", () => {
    const s = play([
      runStart("anthropic"),
      toolCall("write_file", { path: "src/main/notes.md", content: "x" }),
    ]);
  ...
  it("a decision resolves the gate; a denial is an error", () => {
    const allow = play([
      runStart("anthropic"),
      toolCall("write_file", { path: "a.txt" }),
  ...
  it("run_end sends the packet home; error collapses to the user", () => {
    expect(
      play([runStart("anthropic"), { type: "run_end", runId: "r1", stopReason: "end_turn", ts: T }]).focus,
    ).toBe("user");
    const err = play([
      runStart("anthropic"),
      toolCall("write_file", { path: "x" }),
      { type: "error", message: "x", ts: T },
    ]);
  ...
  it("child-agent events keep the parent packet but mark the active child", () => {
    ...
    const child = advanceScene(parent, {
      type: "text_delta",
      agentId: "explore-1",
      text: "…",
      ts: T,
  ...
  it("a subagent's permission gate resolves on the CHILD loop, leaving the main agent untouched", () => {
    ...
    const allowed = advanceScene(pending, {
      type: "permission_decision",
      callId: "cc",
      allowed: true,
      ts: T,
    ...
    const denied = advanceScene(pending, {
      type: "permission_decision",
      callId: "cc",
      allowed: false,
      ts: T,
  ...
  it("usage and unknown additive events do not move the packet", () => {
    ...
    expect(
      advanceScene(before, { type: "usage", agentId: "main", inputTokens: 1, outputTokens: 1, ts: T }),
    ).toEqual(before);
    expect(
      advanceScene(before, {
        type: "voice_input",
