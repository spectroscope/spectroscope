...
describe("eventPreview — a hovered event's type + mini preview", () => {
  it("names the exact wire type and previews the payload per kind", () => {
    expect(eventPreview({ type: "text_delta", agentId: "main", text: "hello ", ts: 1 })).toEqual({
      type: "text_delta",
      detail: "hello",
    });

    expect(eventPreview({ type: "thinking_delta", agentId: "main", text: "let me think", ts: 1 })).toEqual({
      type: "thinking_delta",
      detail: "let me think",
    });
    ...
    expect(eventPreview({ type: "permission_decision", callId: "c1", allowed: true, ts: 1 })).toEqual({
      type: "permission_decision",
      detail: "allowed",
    });
    expect(eventPreview({ type: "permission_decision", callId: "c1", allowed: false, ts: 1 })).toEqual({
      type: "permission_decision",
      detail: "denied",
    });
    ...
    expect(
      eventPreview({ type: "run_start", runId: "r1", agentId: "main", prompt: "Summarize the repo", ts: 1 }),
    ).toEqual({ type: "run_start", detail: "Summarize the repo" });
    expect(eventPreview({ type: "turn_start", agentId: "main", turn: 2, ts: 1 })).toEqual({
      type: "turn_start",
      detail: "turn 2",
    });
    expect(eventPreview({ type: "run_end", runId: "r1", stopReason: "end_turn", ts: 1 })).toEqual({
      type: "run_end",
      detail: "end_turn",
    });
    expect(eventPreview({ type: "error", agentId: "main", message: "boom", ts: 1 })).toEqual({
      type: "error",
      detail: "boom",
    });
    ...
    expect(
      eventPreview({ type: "usage", agentId: "main", inputTokens: 120, outputTokens: 30, ts: 1 }),
