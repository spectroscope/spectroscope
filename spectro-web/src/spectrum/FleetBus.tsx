...
interface FleetBusProps {
  ...
  /** In the control room the card head SELECTS: the detail column is right
   *  there, so a card click must not send the reader to another surface. */
...
function BusAgentCard({
  ...
  epoch: number;
  selected: boolean;
  /** What a click on the card means — select in the room, focus in the variants. */
  ...
  return (
    <div
      ref={cardRef}
      className={`bus-card bus-card--${state}${gateOn ? " bus-card--gate" : ""}${selected ? " bus-card--selected" : ""}`}
      style={style}
    ...
    >
      <button
        ...
        title={onPick ? t(lang, "bus.openAgent") : undefined}
        onClick={onPick ? () => onPick(card.id) : undefined}
      >
...
export function FleetBus({
  ...
  // One door per host: the room selects, the tab variants focus.
  const pick = onSelect ?? onFocusAgent;
  const lang = useLang();
