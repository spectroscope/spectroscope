  ...
  return (
    ...
    >
      <button
        ...
        title={onFocusAgent ? t("open this agent") : undefined}
      ...
      >
        ...
        {epoch > 0 && <span className="bus-card-epoch mono" title={t("restarted")}>#{epoch}</span>}
        {!card.connected && <span className="bus-card-ghost">{t("offline")}</span>}
      </button>
      {card.task !== "" && <p className="bus-card-task" title={card.task}>{card.task}</p>}
      <div className="bus-card-os mono" aria-label={t("inline OS")}>
      ...
      <button
        ...
        title={onOpenTrace ? t("open in trace") : undefined}
      ...
      <div className="bus-card-foot mono tabular">
        <span>
          {graphNode !== undefined ? `${graphNode.inTokens} in · ${graphNode.outTokens} out` : "—"}
        </span>
        {onStop && card.connected && (
          <button
            type="button"
            className="bus-card-stop"
            title={t("stop this node")}
      ...
      <div className="bus-card-composer">
        <input
          type="text"
          className="mono"
          placeholder={t("message this agent…")}
          disabled
          title={t("bus messaging lands with the 0.7 wire verb — the hub speaks stop and gate today")}
        />
        <button type="button" disabled title={t("bus messaging lands with the 0.7 wire verb — the hub speaks stop and gate today")}>
          →
        </button>
      </div>
    </div>
  ...
  if (ordered.length === 0) {
    return (
      <div className="bus-canvas bus-canvas--empty">
        <p className="bus-empty">{t("no nodes on the bus yet")}</p>
        {contextId !== undefined && (
          <FleetSpawnForm contextId={contextId} hubPort={hubPort ?? null} />
        )}
      </div>
    );
  }
    ...
    <div className="bus-canvas">
      <div className="bus-scroll" ref={scrollRef}>
        <div className="bus-cards">
          {ordered.map((n) => {
            ...
            return (
              <div key={n.id} className={n.spawnedBy !== null ? "bus-slot bus-slot--child" : "bus-slot"}>
                {n.spawnedBy !== null && (
                  <span className="bus-slot-parent mono" title={t("spawned by")}>
                    ⌐ {n.spawnedBy}
                  </span>
                )}
                <BusAgentCard
          ...
          {contextId !== undefined && (
            <div className="bus-slot bus-slot--spawn">
              <FleetSpawnForm contextId={contextId} hubPort={hubPort ?? null} />
            </div>
          )}
        </div>
        <svg className="bus-rail" width={railWidth} height="56" aria-hidden="true">
          ...
          <rect className="bus-rail-line" x="0" y="26" width={railWidth} height="2.5" rx="1.25" fill="url(#bus-line)" />
          {traffic.map((m) => {
