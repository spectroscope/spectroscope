// The fleet spawn panel (on the fleet canvas): start a node from the cockpit.
// The web spawn is deliberately READ-ONLY — the endpoint is browser-reachable,
// so it must never launch a node that writes or runs code. The panel is honest
// about that and hands the operator a copy-paste `spectro node …` command for a
// full-power node, built from the same fields plus the live hub port.
      ...
      {!open ? (
        ...
        <div className="fleet-spawn-panel">
          <div className="fleet-spawn-head">
            ...
            <button type="button" className="fleet-spawn-close" onClick={() => setOpen(false)} aria-label={de ? "schließen" : "close"}>×</button>
          </div>
          ...
          <div className="fleet-spawn-row">
            <label className="fleet-spawn-field"><span>context</span>
              <input value={fields.context} onChange={(e) => set("context", e.target.value)} /></label>
