...
export function compile(dsl: Dsl, lang: Lang, baseTs = 1_783_000_000_000): RunEvent[] {
  ...
  const seedContext = (turn: number) => {
    push({
      ...
      turn,
      messages: turn,
      estimatedTokens: estTok(sys.length + TOOL_SCHEMA_CHARS + convoChars),
      threshold: CONTEXT_THRESHOLD_TOKENS,
      parts: [
        {
          label: loc({ en: "system prompt", de: "System-Prompt" }, lang),
          chars: sys.length,
          estTokens: estTok(sys.length),
        },
        {
          label: loc({ en: "tool schemas", de: "Tool-Schemas" }, lang),
          chars: TOOL_SCHEMA_CHARS,
          estTokens: estTok(TOOL_SCHEMA_CHARS),
        },
        {
          label: loc({ en: "conversation", de: "Konversation" }, lang),
          chars: convoChars,
          estTokens: estTok(convoChars),
        },
      ],
    });
  };
  ...
  push({
    ...
    runId,
    agentId: "main",
    prompt: loc(dsl.prompt, lang),
    provider: dsl.provider ?? "ollama",
  });
    ...
    for (const s of steps) {
      if ("think" in s) push({ type: "thinking_delta", agentId: childId, text: loc(s.think, lang) });
        ...
        push({
          ...
          callId,
          name: "report_status",
          input: { message: loc(s.status, lang) },
        });
        push({
          type: "agent_message",
          from: childId,
          ...
          role: "status",
          state: "working",
          text: loc(s.status, lang),
        });
        push({ type: "tool_result", agentId: childId, callId, output: "ok", isError: false, durationMs: 0 });
      } else if ("read" in s)
        diskTool(childId, "read_file", s.read, s.result ? loc(s.result, lang) : undefined, "(file)");
      else if ("write" in s)
        diskTool(
          childId,
          "write_file",
          s.write,
          s.result ? loc(s.result, lang) : undefined,
          "ok, wrote 1 file",
        );
      else if ("list" in s)
        diskTool(childId, "list_dir", s.list, s.result ? loc(s.result, lang) : undefined, "a.ts\nb.ts");
      else if ("run" in s)
        gatedTool(
          childId,
          "run_command",
  ...
  const expandSpawn = (step: Extract<Step, { spawn: string }>) => {
    ...
    push({
      ...
      role: "result",
      state: "completed",
      text: `[${step.spawn}] done`,
    });
    push({
      type: "tool_result",
      agentId: "main",
      callId: parentCall,
      output: `[${step.spawn}] result`,
      isError: false,
      durationMs: SUBAGENT_RESULT_DURATION_MS,
    });
  };
  ...
  const expandFanout = (step: Extract<Step, { fanout: unknown }>) => {
    ...
    push({
      ...
      callId: parentCall,
      name: fo.tool,
      input: { areas: fo.agents.map((a) => a.id) },
    });
    ...
    for (let i = 0; i < maxLen; i++)
      for (const a of fo.agents) if (a.steps[i]) childSteps(a.id, [a.steps[i]]);
    for (const a of fo.agents) {
    ...
    push({
      ...
      agentId: "main",
      callId: parentCall,
      output: `${fo.agents.length} reviews back`,
      isError: false,
      durationMs: SUBAGENT_RESULT_DURATION_MS,
    });
  };
  ...
  for (const step of dsl.steps) {
    lastWasSay = false;
    if ("think" in step) {
      const text = loc(step.think, lang);
      convoChars += text.length;
      push({ type: "thinking_delta", agentId: "main", text });
    } else if ("say" in step) {
      const text = loc(step.say, lang);
      convoChars += text.length;
      ...
      push({
        type: "compaction",
        agentId: "main",
        removedTurns: step.compact.removedTurns,
        summaryChars: step.compact.summaryChars,
      });
      ...
      push({
        ...
        turn: 1,
        messages: 1,
        estimatedTokens: step.context.parts.reduce((s, p) => s + p.estTokens, 0),
        threshold: CONTEXT_THRESHOLD_TOKENS,
        parts: step.context.parts.map((p) => ({
      ...
      diskTool(
        ...
        "read_file",
        step.read,
        step.result ? loc(step.result, lang) : undefined,
        "(file contents)",
      );
    ...
    else if ("list" in step)
      diskTool("main", "list_dir", step.list, step.result ? loc(step.result, lang) : undefined, "a.ts\nb.ts");
    else if ("run" in step)
      gatedTool(
        "main",
        "run_command",
  ...
  if (lastWasSay)
    push({ type: "usage", agentId: "main", inputTokens: estTok(convoChars), outputTokens: 200 });
