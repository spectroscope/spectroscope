  ...
  const handleRecord = (r: CCRecord, i: number): void => {
    ...
    if (r.isSidechain) {
      ...
      if (r.type === "assistant") {
        ...
        // The tokens, for THIS file's own agent only. The standing rule that a
        // subagent's usage becomes no frame is about a session file, where
        // framing it would move that session's total upward and an owner gets
        // to sanction that. Here the agent IS the run: there is no total to
        // move, and leaving it out would be the footer reporting zero for a
        // transcript that recorded its cost on every response. A nested child
        // inside this file keeps the standing rule, unchanged.
        const su = owner === subagentRoot && usageAt.has(i) ? r.message?.usage : undefined;
        if (su)
          out.push({
            type: "usage",
            agentId: owner,
            inputTokens: su.input_tokens ?? 0,
            outputTokens: su.output_tokens ?? 0,
            ...(su.cache_read_input_tokens !== undefined
              ? { cacheReadTokens: su.cache_read_input_tokens }
              : {}),
            ...(su.cache_creation_input_tokens !== undefined
              ? { cacheCreationTokens: su.cache_creation_input_tokens }
              : {}),
            ts,
          });
        return;
    ...
    if (r.type === "user") {
      ...
      announce(modelOf(r), ts);
      noteStop(rootId, r);
      if (startsTurn(i)) out.push({ type: "turn_start", agentId: rootId, turn: nextTurn(rootId), ts });
      for (const b of blocks) emitBlock(rootId, b, ts);
