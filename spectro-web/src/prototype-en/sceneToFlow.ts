...
import type { DiskState, Focus, GateState, Scene, SubagentInfo } from "../lab/labScene";
...
export function sceneToFlow(
  scene: Scene,
  detail: Detail,
  opts: { local: boolean; provider: string; model: string; systemPrompt?: string },
): FlowResult {
  ...
  for (const z of L.zones) {
    ...
    nodes.push({
      ...
      data: { variant: z.variant, label: z.label },
      draggable: false,
      selectable: false,
      zIndex: 0,
  ...
  if (L.boundary) {
    nodes.push({
      ...
      data: { variant: "boundary", label: "NETWORK BOUNDARY" },
      draggable: false,
      selectable: false,
      zIndex: 1,
  ...
  // ----- LLM -----
  N("llm", "llm", {
    active: scene.focus === "llm",
    local: opts.local,
    provider: opts.provider,
    model: opts.model,
    think: detail.think["main"] ?? "",
    answer: detail.answer["main"] ?? "",
  });

  // ----- external services -----
  N("netz", "ext", { kind: "netz", active: mcpInUse });
  N("mcpserver", "ext", { kind: "mcpserver", active: mcpInUse, mcp: scene.activeMcp });
  ...
  subs.forEach((c, i) => {
    ...
    const act = activity(c.focus, c.disk, c.activeFile, c.activeCommand, c.activeMcp, c.gate);
    N(id, "subagent", {
      id: c.id,
      label: c.label,
      task: c.task,
      state: c.state,
      stateLabel: LIFECYCLE[c.state],
  ...
  //   agent → MCP-client → network stack →⟂ Netz → MCP-server
  const mcpErr = scene.isError && mcpInUse;
  E("e-agent-osmcp", "agent", "os-mcp", "bs", "tt", mainLit === "os-mcp" || mcpInUse, { err: mcpErr });
