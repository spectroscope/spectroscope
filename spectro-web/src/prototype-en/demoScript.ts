...
const raw: RunEvent[] = [
  ...
  },
  {
    type: "thinking_delta",
    agentId: "main",
    text: "The user wants a plan. I delegate it to a planner subagent via build_plan.",
    ts: 0,
  },
  {
    type: "tool_call",
    ...
    name: "build_plan",
    input: {
      task: 'Plan how to add a --version flag to the Java CLI "spectroscope". Maximum 5 steps. Do not write files, just the plan as text.',
    },
    ts: 0,
  },
  ...
  { type: "agent_spawn", agentId: "worker-1", parentId: "main", task: CHILD_TASK, ts: 0 },
  {
    ...
    role: "task",
    state: "submitted",
    text: 'Plan how to add a --version flag to the Java CLI "spectroscope". Maximum 5 steps.',
    label: "build_plan",
    ts: 0,
  },
  ...
  { type: "turn_start", agentId: "worker-1", turn: 1, ts: 0 },
  {
    type: "thinking_delta",
    agentId: "worker-1",
    text: "First I load the 'writing-plans' skill to follow the format.",
    ts: 0,
  },
  {
    type: "tool_call",
  ...
  },
  {
    ...
    callId: C.skill,
    output:
      "# Writing plans\n\nUse this skill when a design exists and the next artifact is a PLAN. Keep steps small, verifiable, ordered …",
    isError: false,
    durationMs: 2,
    ts: 0,
  },
  {
    ...
    callId: C.status1,
    name: "report_status",
    input: { message: "Reading task and existing files" },
    ts: 0,
  },
  {
    type: "agent_message",
    ...
    role: "status",
    state: "working",
    text: "Reading task and existing files",
    ts: 0,
  },
  {
    type: "tool_result",
  ...
  },
  {
    type: "thinking_delta",
    agentId: "worker-1",
    text: "Now I draft the 5 steps: picocli @Option, VersionProvider, Manifest, test, docs.",
    ts: 0,
  },
  {
    type: "agent_message",
  ...
  },
  {
    type: "text_delta",
    agentId: "worker-1",
    text: '# Plan: --version flag for the spectroscope CLI\n\n1. picocli @Option(names="--version", versionHelp=true) …',
    ts: 0,
  },
  { type: "run_end", runId: CHILD_RUN, stopReason: "end_turn", ts: 0 },
  {
    ...
    role: "result",
    state: "completed",
    text: "[worker-1] result (tokens: 4433 in / 1447 out):\n# Plan: --version flag for the spectroscope CLI\n\n1. picocli @Option …",
    ts: 0,
  },
  ...
  // ---- main receives the plan, resumes ----
  {
    ...
    agentId: "main",
    callId: C.plan,
    output: "[worker-1] result (tokens: 4433 in / 1447 out):\n# Plan: --version flag …",
    isError: false,
    durationMs: 29467,
    ts: 0,
  },
  ...
  },
  {
    type: "thinking_delta",
    agentId: "main",
    text: "The plan is solid. I quickly verify the current Main class before presenting it.",
    ts: 0,
  },
  ...
  // ---- disk read (permission-free) ----
  {
    ...
    callId: C.read,
    name: "read_file",
    input: { path: "spectro-cli/src/main/java/dev/spectroscope/cli/Main.java" },
    ts: 0,
  },
  {
    type: "tool_result",
    agentId: "main",
    callId: C.read,
    output: "public final class Main { public static void main(String[] a) { … } }",
    isError: false,
    durationMs: 3,
    ts: 0,
  },
  ...
  // ---- approved run_command gate ----
  {
    ...
    callId: C.cmd,
    name: "run_command",
    input: { command: "./gradlew test --quiet" },
    ts: 0,
  },
  {
  ...
  // ---- denied MCP gate (red) ----
  {
    ...
    input: { query: "version flag conventions", limit: 5 },
    ts: 0,
  },
  {
    type: "permission_request",
  ...
  { type: "permission_decision", callId: C.mcp, allowed: false, ts: 0 },
  {
    ...
    agentId: "main",
    callId: C.mcp,
    output: "ERROR: the user denied the execution.",
    isError: true,
    durationMs: 334,
    ts: 0,
  },
  ...
  // ---- final answer ----
  {
    type: "thinking_delta",
    agentId: "main",
    text: "Even without the notes the plan is enough. I present it to the user.",
    ts: 0,
  },
  {
    type: "text_delta",
    agentId: "main",
    text: "Here is the finished 5-step plan for the --version flag in the spectroscope CLI …",
    ts: 0,
  },
...
export function eventLabel(e: RunEvent): string {
  switch (e.type) {
    case "run_start":
      return `run_start · ${e.agentId}`;
    case "turn_start":
      return `turn_start · ${e.agentId} #${e.turn}`;
    case "context_info":
      return `context_info · ${e.estimatedTokens} tok`;
    case "thinking_delta":
      return `thinking · ${e.agentId}`;
    case "text_delta":
      return `text · ${e.agentId}`;
    case "tool_call":
      return `tool_call · ${e.name}`;
    case "permission_request":
      return `permission_request · ${e.name}`;
    case "permission_decision":
      return `permission_decision · ${e.allowed ? "allow" : "deny"}`;
    case "tool_result":
      return `tool_result${e.isError ? " · error" : ""}`;
    case "agent_spawn":
      return `agent_spawn · ${e.agentId}`;
    case "agent_message":
      return `agent_message · ${e.role}`;
    case "usage":
      return `usage · ${e.agentId}`;
    case "run_end":
      return `run_end · ${e.stopReason}`;
    case "error":
      return `error`;
    default:
      return (e as RunEvent).type;
  }
}
...
const rawFanout: RunEvent[] = [
  ...
  },
  {
    type: "thinking_delta",
    agentId: "main",
    text: "I fan out into three parallel reviewers: bugs, performance, security.",
    ts: 0,
  },
  {
    type: "tool_call",
    agentId: "main",
    callId: "call-review-fanout",
    name: "review",
    input: { areas: ["bugs", "performance", "security"] },
    ts: 0,
  },
...

  {
    ...
    agentId: "bugs",
    parentId: "main",
    task: "Check the diff for bugs: null checks, off-by-one, error paths.",
    ts: 0,
  },
  {
    type: "agent_message",
  ...
  },
  {
    ...
    agentId: "perf",
    parentId: "main",
    task: "Check the diff for performance issues: N+1 queries, unnecessary allocations.",
    ts: 0,
  },
  {
    type: "agent_message",
  ...
  },
  {
    ...
    agentId: "security",
    parentId: "main",
    task: "Check the diff for security: injection, secrets, path traversal.",
    ts: 0,
  },
  {
    type: "agent_message",
...

  {
    type: "tool_call",
    agentId: "security",
    callId: "call-sec-read",
    name: "read_file",
    input: { path: "src/main/java/app/Db.java" },
    ts: 0,
  },
  {
    type: "agent_message",
  ...
  },
  {
    ...
    agentId: "security",
    callId: "call-sec-read",
    output: 'String sql = "SELECT * FROM u WHERE id=" + id;',
    isError: false,
    durationMs: 3,
    ts: 0,
  },
...

  {
    type: "text_delta",
    agentId: "bugs",
    text: "## Bugs\n- Off-by-one in the pager (last page missing).",
    ts: 0,
  },
  { type: "text_delta", agentId: "perf", text: "## Performance\n- N+1 query in ListRepo.findAll().", ts: 0 },
  {
    type: "text_delta",
    agentId: "security",
    text: "## Security\n- SQL concat in Db.java → injection.",
    ts: 0,
  },
  ...
  { type: "run_end", runId: "run-bugs", stopReason: "end_turn", ts: 0 },
  {
    ...
    role: "result",
    state: "completed",
    text: "[bugs] 1 bug: off-by-one in the pager.",
    ts: 0,
  },
  { type: "run_end", runId: "run-perf", stopReason: "end_turn", ts: 0 },
  {
    ...
    role: "result",
    state: "completed",
    text: "[perf] 1 N+1 query in ListRepo.",
    ts: 0,
  },
  { type: "run_end", runId: "run-security", stopReason: "end_turn", ts: 0 },
  {
    ...
    role: "result",
    state: "completed",
    text: "[security] SQL concat → injection risk in Db.java.",
    ts: 0,
  },

  {
    ...
    agentId: "main",
    callId: "call-review-fanout",
    output: "3 reviews back: 1 bug, 1 perf, 1 security.",
    isError: false,
    durationMs: 21400,
    ts: 0,
  },
  ...
  },
  {
    type: "thinking_delta",
    agentId: "main",
    text: "All three reviews are back. I prioritize the security finding.",
    ts: 0,
  },
  {
    type: "text_delta",
    agentId: "main",
    text: "Summary: 1 critical security finding (SQL injection), 1 bug, 1 performance issue …",
    ts: 0,
  },
...
export const DEMO_EVENTS_FANOUT: RunEvent[] = rawFanout.map((e, i) => ({
  ...e,
  ts: 1783430000000 + i * 1200,
}));
...
const rawPermission: RunEvent[] = [
  {
    ...
    runId: "run-perm",
    agentId: "main",
    prompt: "Clean up the data/tmp directory, then show me the git status.",
    provider: "ollama",
    ts: 0,
  },
  { type: "turn_start", agentId: "main", turn: 1, ts: 0 },
  ...
  },
  {
    type: "text_delta",
    agentId: "main",
    text: "I deleted nothing (the deletion was denied). Git shows one changed file.",
    ts: 0,
  },
...
const rawDisk: RunEvent[] = [
  {
    ...
    runId: "run-disk",
    agentId: "main",
    prompt: "Read src/config.json, write the updated version, list src/ and run the tests.",
    provider: "ollama",
    ts: 0,
  },
  { type: "turn_start", agentId: "main", turn: 1, ts: 0 },
  ...
  },
  {
    type: "text_delta",
    agentId: "main",
    text: "Config updated (retries 3→5), src/ listed, tests green.",
    ts: 0,
  },
...
export const DEMO_EVENTS_PERMISSION: RunEvent[] = rawPermission.map((e, i) => ({
  ...e,
  ts: 1783440000000 + i * 1200,
}));
