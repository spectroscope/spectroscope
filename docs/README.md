...
| [HEADLESS-MCP.md](HEADLESS-MCP.md) | **proposed, not built** — what an unattended run may reach. Why `spectro doctor` calls an MCP server reachable while `spectro run`, a cron fire and a fleet node carry none of its tools, what a stdio server costs measured rather than guessed, four roads with the failure each invites, and a recommendation with its bill named. Owner decision pending (card 220) |
...
| [sample-runs/](sample-runs/README.md) | one **shipped example run** as JSONL — the declared 5-phase, 13-agent `workflow-phases` scenario, generated from its own source (`npm run generate:sample-run` in `spectro-web/`) and pinned byte-for-byte against it. Drop it into the import dialog to replay a real multi-agent stream with no key, no server and no model |
