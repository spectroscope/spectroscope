...
| [HEADLESS-MCP.md](HEADLESS-MCP.md) | **proposed, not built** — what an unattended run may reach. Why `spectro doctor` calls an MCP server reachable while `spectro run`, a cron fire and a fleet node carry none of its tools, what a stdio server costs measured rather than guessed, three roads with the failure each invites, and a recommendation with its bill named. Owner decision pending (card 213) |
