...
implementation("dev.spectroscope:spectro-orchestrator:0.8.0")   // fleets
```
...
Four routes to 0.8.0, each with the platform it covers. Every asset is on the
...
**The disk image — macOS on Apple silicon.** `spectroscope-0.8.0-arm64.dmg`
...
not use apt, `spectroscope-0.8.0-x86_64.AppImage` is the same kit as one file.
...
`spectro-server-0.8.0.jar` and run them on a JDK 21; that is the smallest way
