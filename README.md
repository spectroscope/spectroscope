...
implementation("dev.spectroscope:spectro-orchestrator:0.7.0")   // fleets
```
...
Four routes to 0.7.0, each with the platform it covers. Every asset is on the
...
**The disk image — macOS on Apple silicon.** `spectroscope-0.7.0-arm64.dmg`
...
not use apt, `spectroscope-0.7.0-x86_64.AppImage` is the same kit as one file.
...
desktop kit, and no macOS route will help. Take `spectro-0.7.0.zip` (the CLI) or
`spectro-server-0.7.0.jar` and run them on a JDK 21; that is the smallest way
